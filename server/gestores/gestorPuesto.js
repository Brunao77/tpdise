...
export async function deletePuesto(req, res) {
  try {
    ...
    const puestoDAO = new PuestoDAO();
    const puesto = await puestoDAO.deletePuesto(codigo);
  ...
  try {
    ...
    ponderaciones = await Promise.all(
      ponderaciones.map(async (ponderacion) => {
        ...
        ponderacion.factores = await factorDAO.getFactores(
          ponderacion.idCompetencia
        );
        return ponderacion;
      })
    );
    for (let ponderacion of ponderaciones) {
      await Promise.all(
        ponderacion.factores.map(async (factor) => {
          ...
          else if (ponderacion.esEvaluable == undefined)
            ponderacion.esEvaluable = false;
          return;
        })
      );
    }
...
export async function crearPuestoClon(codigo) {
  try {
    ...
    for (const ponderacion of puesto.ponderaciones) {
      const { competencia, factores } = await crearCompetenciaClon(
        ponderacion.idCompetencia
      );
